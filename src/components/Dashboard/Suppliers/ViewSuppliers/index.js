...
const ViewSuppliers = ({ setViewSupplier }) => {
    const {
        toggle: toggleSupplierDrawer,
    ...
    const toggleEditSupplierHandler = (id, supplier) => {
        ...
        <>
            <Container>
                <div className='px-6 pt-4 pb-2 border-b flex flex-row gap-3'>
                    <button className='rounded-full border w-8 h-8 flex justify-center items-center hover:bg-secondary-50' onClick={() => setViewSupplier(false)}>
                        <Icon name='arrow-long-left' className='w-4 h-4' />
                    </button>
                    <h1 className='font-semibold text-2xl' data-toggle='tooltip' data-placement='top' data-text='Product details and all can be seen here'>Supplier</h1>
                </div>
                        ...
                        <div className='bg-white relative rounded-b-lg rounded-t-0'>
                            <div className='flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4'>
                                ...
                                <div className='w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0'>
                                    {
                                        role === 'manager' && (
                                            <button type='button' onClick={() => toggleSupplierDrawer('add-supplier-data')} className='flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2'>
                                    ...
                                    <div className='flex items-center space-x-3 w-full md:w-auto'>
                                        ...
                                        >
                                            <ul className='py-1 text-sm text-secondary-700'>
                                                {
                                                    sortByKey.filter((keyName) => keyName !== '_id' && keyName !== 'password' && keyName !== '__v' && keyName !== 'updatedOn' && keyName !== 'email')
                                                        .map((keyName, index) => (
                                                            <li key={index}>
                                                    ...
                                                    {sortedData.map((supplier) => {
                                                        ...
                                                        return (
                                                            <tr className='border-b' key={supplier._id}>
                                                                <th scope='row' className='px-4 py-3 font-medium text-secondary-900 whitespace-nowrap capitalize'>{supplier.name}</th>
                                                                <td className='px-4 py-3 capitalize'>{supplier.email}</td>
                                                                <td className='px-4 py-3 capitalize'>{supplier.address}</td>
                                                                <td className='px-4 py-3 capitalize'>{supplier.phoneNo}</td>
                                                                <td className='px-4 py-3 capitalize'>{supplier.companyName}</td>
                                                                <td className='px-4 py-3 capitalize'>{supplier.category}</td>
                                                                <td className='px-4 py-3 capitalize'>{supplier.status === 'active' ? 'Active' : 'Inactive'}</td>
                                                                            ...
                                                                            <ul>
                                                                                <li>
                                                                                    <button
                                                                                        ...
                                                                                        onClick={() => toggleEditSupplierHandler(drawerId, supplier)}
                                                                                    >
                                                                                        Edit
                                                                                    </button>
                                                                                </li>
